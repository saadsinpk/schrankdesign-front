...
export const EditPlatesTypes = async (values, id) => {
    ...
    try {
        const data = await put(`/api/v1/platestypes/updateplatesTypes/${id}`, formdata);
        return { data, error: null };
    } catch (error) {
...
export const CreatePartList = async (viewdata, PartListArray) => {
    ...
    if (PartListArray?.length > 0 && Array.isArray(PartListArray)) {
        PartListArray.forEach((part, index) => {
            formdata.append(`child_name[${index}]`, part?.child_name?.[0] ?? part?.name);
            formdata.append(`child_config_id[${index}]`, part?.child_config_id?.[0] ?? part?.configId);
            ...
            if (part?.add_distance?.length > 0 && Array.isArray(part?.add_distance) && part?.functions == "Add-Distance") {
                part?.add_distance.forEach((functionitems, childindex) => {
                    formdata.append(`functions_distance[${index}][${childindex}]`, functionitems?.functions_distance?.[0] ?? functionitems?.Distance);
                    formdata.append(`functions_from[${index}][${childindex}]`, functionitems?.functions_from?.[0] ?? functionitems?.From);
                    formdata.append(`functions_quantity[${index}][${childindex}]`, functionitems?.functions_quantity?.[0] ?? functionitems?.Quantity);
                    formdata.append(`functions_to[${index}][${childindex}]`, functionitems?.functions_to?.[0] ?? functionitems?.To);
                })
            }
...
export const CreateTestPartList = async (SearchingInput, PlateDepth, PlateLength, MaterialName) => {
    let formdata = new FormData();
...
export const CreateCalculation = async (calculation, route, total) => {
    ...
    if (calculation.material_items.length > 0 && Array.isArray(calculation?.material_items)) {
        calculation?.material_items.map((item, index) => {
            if (!shouldContinue) return;
            // Condition 1
            if ((item?.type === "addnumber") && (!item?.value || item?.value === null || item?.value === undefined)) {
            ...
            if (item?.type == "addvariable") {
                if ((item?.type === "addvariable") && (!item?.variable_name || item?.variable_name === null || item?.variable_name === undefined)) {
                    toast.error("Fill all Fields!.");
                    shouldContinue = false;
                    return;
                }
                formdata.append(`material_items[${index}][type]`, item?.type);
                formdata.append(`material_items[${index}][variable_name]`, item?.variable_name);
                formdata.append(`material_items[${index}][config_id]`, item?.config_id);
                formdata.append(`material_items[${index}][test_result]`, item?.test_total || item?.test_result);
                item?.VariableType && formdata.append(`material_items[${index}][VariableType]`, item?.VariableType);
            }
            if (item?.type == "addfunction") {
                if ((item?.type === "addfunction") && (!item?.variable_name || item?.variable_name === null || item?.variable_name === undefined)) {
                    toast.error("Fill all Fields!.");
                    shouldContinue = false;
                    return;
                }
                ...
                formdata.append(`material_items[${index}][variable_name]`, item?.variable_name);
                formdata.append(`material_items[${index}][config_id]`, item?.config_id);
                formdata.append(`material_items[${index}][test_result]`, item?.test_result || item?.test_total);
            }
        })
    }
    if (shouldContinue) {
