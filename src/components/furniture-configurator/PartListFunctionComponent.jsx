...
function PartListFunctionComponent({ PartListArray, setPartListArray, index, setParlistFunctionOpen }) {



    const dropdownOptionsEdge = [
        { label: "Length", icon: null },
        { label: "Deepth", icon: null },
    ]
    ...
    const HandleInputChanges = (inputVal, currentIndex, type) => {
        ...
        updatedPartListArray[index].add_distance[currentIndex] = selected_funtion_Row
        setPartListArray(updatedPartListArray);
    }


    return (
        PartListArray[index]?.add_distance?.length > 0 && Array.isArray(PartListArray[index]?.add_distance) &&
        < div className="toggler  transition duration-300 delay-200 rounded-[5px] col-span-12 bg-[#F6F6F6]  z-[15] absolute w-full top-[102%]  border border-solid border-black px-2 " >
            <div className="flex justify-between  py-6 px-3">
                ...
                <div className="flex flex-row gap-[5px]">
                    <div className="shadow-lg">
                        <button onClick={HandleAddFunction} type="button" className="font-[karla]  text-white font-bold bg-[#456779] rounded-[5px] px-[9px] py-[4px] shadow-lg">
                            Add
                        </button>
                    </div>
                    <div className="shadow-lg">
                        <button onClick={(e) => setParlistFunctionOpen(false)} type="button" className="font-[karla] text-white font-bold bg-[#36695C] rounded-[5px] px-[9px] py-[4px] shadow-lg">
                            Save
                        </button>
                    </div>
                </div>
            ...
            {PartListArray[index]?.add_distance?.length > 0 && Array.isArray(PartListArray[index]?.add_distance) &&
                PartListArray[index]?.add_distance?.map((item, indexinner) => {
                    return (
                        <div key={indexinner} className="flex flex-row items-center flex-wrap justify-between w-full p-3 mb-3  rounded-[5px]  border border-solid border-black ">
                            <div className=" px-4 flex flex-row items-center flex-wrap gap-[15px] justify-between">
                                <h1 className="font-[karla] text-[20px] font-medium">Distance </h1>
                                <div className="sm:w-[170px] border border-solid border-black bg-[#F6F6F6] rounded-[3px]">
                                    <Select
                                        IconComponent={() => <DropDownIcon />}
                                        onChange={(e) => HandleInputChanges(e, indexinner, "Distance")}
                            ...
                            <div className=" px-4 flex flex-row items-center flex-wrap gap-[15px] justify-between">
                                <h1 className="font-[karla] text-[20px] font-medium">From </h1>
                                <div className="sm:w-[170px] border border-solid border-black bg-[#F6F6F6] rounded-[3px]">
                                    <input
                                        ...
                                        onChange={(e) => HandleInputChanges(e, indexinner, "From")}
                            ...
                            <div className=" px-4 flex flex-row items-center flex-wrap gap-[15px] justify-between">
                                <h1 className="font-[karla] text-[20px] font-medium">Quantity </h1>
                                <div className="sm:w-[170px] border border-solid border-black bg-[#F6F6F6] rounded-[3px]">
                                    <input
                                        className="block w-100 h-100 text-[20px] font-[karla] font-bold  outline-none w-full border border-solid border-black bg-[#F6F6F6] rounded-[3px] px-3"
                                        onChange={(e) => HandleInputChanges(e, indexinner, "Quantity")}
                            ...
                            <button type='button' onClick={() => HandleDeleteFunction(item._id, index)} className="w-[31px] shrink-0" >
...
const DropDownIcon = () => (
    <svg width="66" height="43" viewBox="0 0 29 23" fill="none" xmlns="http://www.w3.org/2000/svg">
