  ...
  return (
    <>
      {showDimensions &&
        dimensionInfo.map((item, key) => (
          <group key={key}>
            <DimensionLine
              points={[
                [item.x, item.top, depth + 2.3],
                [item.x, item.bottom, depth + 2.3],
              ]}
              endDir="X"
              value={parseInt(item.top - item.bottom)}
              center={[item.x, (item.top + item.bottom) / 2, depth + 2.3]}
              lineWidth={1.8}
              isBig={false}
            />
          </group>
      ...
      <DimensionLine
        points={[
          [width + 15, 0, depth],
          [width + 15, height, depth],
        ]}
        endDir="X"
        value={parseInt(height)}
        center={[width + 15, height / 2, depth]}
        lineWidth={2.5}
        isBig={true}
      />
