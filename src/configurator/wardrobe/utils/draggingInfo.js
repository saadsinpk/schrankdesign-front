...
const getDefaultScale = (type, drawerHeight, depth) => {
  const furnishingConfigs = {
    ...
    [Config.furnishing.type.pantsPullout]: {
      width:
        (Config.furnishing.pantsPullout.minWidth +
          Config.furnishing.pantsPullout.maxWidth) /
          2 -
        Config.furnishing.default.spaceSides * 2,
      height: Config.furnishing.pantsPullout.thickness1,
      depth:
        depth -
        Config.furnishing.default.spaceFront -
        Config.plate.backThickness -
...
const getDrawerDepth = (type, depth) => {
  ...
  const i = depthRange.findIndex((depth) => depth >= drawerDepthLimit)
  return depthRange[i !== -1 ? i : depthRange.length - 1] // Returns the last depth if no depth found
}
