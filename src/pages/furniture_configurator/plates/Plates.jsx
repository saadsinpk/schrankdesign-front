...
import { GetallPlates } from "../../../Functions-configurator/Function-configurator";
  ...
  return (
    <Layout>
      <SubLayout tabs={tabs}>
        ...
        {isLoading && <Loader />}
        {NewPlate && < Accordian
          afterCollapse={<PlatesAccordianAfter newPlate={setPlatesList} createplate={true} />}
          add={true}
        ...
        {PlatesList?.length > 0 && Array.isArray(PlatesList) && PlatesList?.map((item, index) => {
          return (
            <Accordian
              key={index}
              afterCollapse={<PlatesAccordianAfter newPlate={setPlatesList} viewdata={item} />}
